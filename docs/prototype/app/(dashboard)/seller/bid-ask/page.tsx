...
// ─── Main page ────────────────────────────────────────────────────────────────
export default function SellerBidAskPage() {
  const { message, modal } = App.useApp();
  const [rubberType, setRubberType] = useState<string>('RSS3');
  const [viewMode, setViewMode]     = useState<'all' | 'mine'>('all');
  const [bids] = useState<Offer[]>(INITIAL_BIDS);
  const [asks, setAsks] = useState<Offer[]>(INITIAL_ASKS);
  const [open, setOpen] = useState(false);
  const [form] = Form.useForm();

  // Matching state — seeded from shared mock, filtered to matches where seller = "me"
  const [matches, setMatches] = useState<Match[]>(() => myMatches('seller', MOCK_MATCHES));
  ...
  const pendingMatchCount = useMemo(() => matches.filter((m) => {
    ...
    if (stage === 'awaiting_counterparty' && !m.sellerMatchedAt) return true;
    if (stage === 'contract_drafted' && !m.sellerSignedAt) return true;
    // seller doesn't pay — only monitors
    return false;
  }).length, [matches]);
  ...
  // Delete one of my own asks (only allowed before any contract stage).
  // Once a row matches, it moves to "การจับคู่ของฉัน" (Match) which has its
  // own delete rules — blocked once the contract is signed.
  const handleDeleteAsk = (offer: Offer) => {
    if (!offer.mine) return;
    modal.confirm({
      title: 'ลบคำเสนอขาย?',
      content: `ต้องการลบคำเสนอขาย ${offer.rubberType} ราคา ${offer.price.toFixed(2)} ฿/กก. ปริมาณ ${offer.quantity.toLocaleString()} กก. หรือไม่?`,
      okText: 'ลบ',
      okButtonProps: { danger: true },
      cancelText: 'ยกเลิก',
      onOk: () => {
        setAsks(prev => prev.filter(a => a.id !== offer.id));
        message.success('ลบคำเสนอขายแล้ว');
      },
    });
  };

  // Create offer → add to Ask table (seller side).
  ...
  const handleCreate = (values: { rubberType: string; price: number; quantity: number }) => {
    const duplicate = asks.some(
      a => a.rubberType === values.rubberType &&
           Math.abs(a.price - values.price) < 0.001
    );
    setAsks(prev => [...prev, {
      id: `A-${Date.now()}`,
  ...
  const boardCard = (
    ...
    >
        ...
        <Row gutter={[12, 12]}>
          <Col xs={24} md={12}>
            <SideColumn key={`bid-${rubberType}-${viewMode}`} side="bid" offers={filteredBids} maxQty={maxQty} />
          </Col>
          <Col xs={24} md={12}>
            <SideColumn key={`ask-${rubberType}-${viewMode}`} side="ask" offers={filteredAsks} maxQty={maxQty} onDelete={handleDeleteAsk} />
          </Col>
        </Row>

        {/* Hint */}
        <div style={{ marginTop: 12, textAlign: 'center', fontSize: 12, color: '#8c8c8c' }}>
          คำเสนอของคุณจะถูกเพิ่มในฝั่ง <span style={{ color: ASK_COLORS.priceText, fontWeight: 600 }}>เสนอขาย (Ask)</span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Modal
        ...
        title={
          <span>
            <PlusOutlined style={{ marginRight: 8, color: ASK_COLORS.priceText }} />
            สร้างคำเสนอขาย (Ask)
          </span>
        }
